    ...
    return Signal { subscriber in
        ...
        queue.async {
            ...
            let seedAddressList: [Int: [String]] = [
                2: ["mtproto.im.mixlink.cc"]
            ]

            for (id, ips) in seedAddressList {
                context.setSeedAddressSetForDatacenterWithId(id, seedAddressSet: MTDatacenterAddressSet(addressList: ips.map {
                    MTDatacenterAddress(ip: $0, port: 2398, preferForMedia: false, restrictToTcp: true, cdn: false, preferForProxy: false, secret: nil)
                }))
            }
